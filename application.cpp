...
{
    GLuint program = 0, vertex_id = 0, fragment_id = 0;
	...
	glShaderSource(vertex_id,1,&vertex_file_list,NULL);
	glCompileShader(vertex_id);
	
	fragment_id = glCreateShader(GL_FRAGMENT_SHADER);
	glShaderSource(fragment_id,1,&fragment_file_list,NULL);
    ...
    debugShader(vertex_id, fragment_id, program);
    return program;
}

application::application()
    :solid(true), lightAngle(0), program(0), right_button_down(false),
    initial_x(0), material_index(0)
{
}
